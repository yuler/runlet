module AccountSlug
  ...
  RESERVED_SLUGS = %w[session dashboard onboarding dev admin api assets billing help jobs login logout magic_link rails settings setup static support runlet device cable]
  ...
  class Extractor
    ...
    def call(env)
      ...
      if request.path_info =~ %r{\A(/api/v\d+/accounts)/(#{AccountSlug::PATTERN})} && !$2.in?(RESERVED_SLUGS)
        env["runlet.account_slug"] = AccountSlug.decode($2)
        request.path_info = "#{$1}#{$'}"
      # Skip slug extraction for API paths to prevent treating path segments as account slugs.
      elsif request.path_info.start_with?("/api")
        env["runlet.account_slug"] = nil
      ...
      Rails.logger.info({ tag: "AccountSlug", slug: env["runlet.account_slug"], path: env["PATH_INFO"] }.to_json)

      # Set the account in the current thread
      if env["runlet.account_slug"]
        account = Account.find_by(slug: env["runlet.account_slug"])
