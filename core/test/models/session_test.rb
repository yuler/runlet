...
class SessionTest < ActiveSupport::TestCase
  test "user_agent_summary correctly identifies macOS desktop app" do
    session = Session.new(user_agent: "Runlet Desktop/0.1.0 (macOS)")
    assert_equal "Runlet Client on macOS", session.user_agent_summary
  end

  test "desktop_version returns correct desktop version tag" do
    session = Session.new(kind: :desktop, user_agent: "Runlet Desktop/0.1.0 (macOS)")
